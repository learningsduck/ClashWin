...
1. 已部署 **ClashFeng 认证 API**（**HTTPS 域名**，勿对公网开放 3001 端口），参见 [ClashFeng-management-install](https://github.com/learningsduck/ClashFeng-management-install) 安装说明。
...
| 项 | 要求 |
|----|------|
| API 地址 | 仅 `https://域名`，禁止 `http://IP` 或 `:3001` |
| 服务端 | `REQUIRE_HTTPS=true`，Node 绑定 `127.0.0.1`，安全组只开 80/443 |
| endpoints.json | 配置 `primary` + 至少一个 `backups` 备用域名 |
| 客户端构建 | `.env` 中 `VITE_AUTH_API_PRIMARY` / `BACKUPS` 与线上一致 |
| 用户异常 | 登录页「清除已保存的登录信息」可重置 API 线路缓存 |
