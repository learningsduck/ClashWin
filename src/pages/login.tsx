...
const LoginPage = () => {
  ...
  const [authApiBase, setAuthApiBase] = useState("");

  const showLocalAdminLogin = import.meta.env.DEV;
  ...
  useEffect(() => {
    initAuthEndpoints()
      .then(() => setAuthApiBase(getActiveAuthBaseUrl()))
      .catch(() => setAuthApiBase(getActiveAuthBaseUrl()));
  }, []);
  ...
  const handleClearSavedLogin = async () => {
    ...
    setSendCodeError("");
    try {
      await initAuthEndpoints();
    } catch {
      /* ignore */
    }
    setAuthApiBase(getActiveAuthBaseUrl());
  };
        ...
        >
          ...
          {authApiBase ? (
            <Typography
              ...
              title={authApiBase}
            >
              {t("auth.login.currentApi", { url: authApiBase })}
            </Typography>
          ) : null}

          <Box sx={{ mt: 2, pt: 2, borderTop: 1, borderColor: "divider" }}>
            {showLocalAdminLogin && !showAdminForm ? (
              <Link
                component="button"
                variant="caption"
