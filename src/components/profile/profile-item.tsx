...
import { isDeveloperModeUnlocked } from "@/utils/developer-mode";
...
export const ProfileItem = (props: Props) => {
  ...
  const basicProfileMenuLabels = new Set<TranslationKey>([
    menuLabels.select,
    menuLabels.update,
    menuLabels.delete,
  ]);
  ...
  const visibleContextMenu = (
    devModeUnlocked
      ? hasUrl
        ? urlModeMenu
        : fileModeMenu
      : (hasUrl ? urlModeMenu : fileModeMenu).filter((item) =>
          basicProfileMenuLabels.has(item.label),
        )
  );
    ...
    >
      <ProfileBox
        ...
        onContextMenu={(event) => {
          ...
          setDevModeUnlocked(isDeveloperModeUnlocked());
