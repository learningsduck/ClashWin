...
import { pullSubscribeFromServerAndImport } from "@/services/subscribe-sync";
...
export const HomeProfileCard = ({
  ...
  const onAutoFetchSubscribe = useLockFn(async () => {
    if (!user?.id) {
    ...
    try {
      await pullSubscribeFromServerAndImport(user);
      onProfileUpdated?.();
      refreshAll();
    } finally {
    ...
    return (
      <Stack direction="row" spacing={1} alignItems="center">
        <Button
          ...
          onClick={() => void onAutoFetchSubscribe()}
          sx={{ borderRadius: 1.5 }}
        >
          {t("profiles.page.actions.autoFetchSubscribe")}
        </Button>
        {current ? (
          <Button
            variant="outlined"
  ...
  }, [autoFetching, current, goToProfiles, onAutoFetchSubscribe, t]);
