...
const DEVELOPER_OPTION_PASSWORD = "zjy688";
...
const SettingClash = ({ onError }: Props) => {
  ...
  return (
    ...
    <SettingList title={t("settings.sections.clash.title")}>
      ...
      >
        {t("settings.sections.clash.shortcuts.groupTitle")}
      </Typography>
      <SettingItem
        onClick={() => navigate("/connections")}
        label={t("settings.sections.clash.shortcuts.connections")}
      />
      <SettingItem
        onClick={() => navigate("/rules")}
        label={t("settings.sections.clash.shortcuts.rules")}
      />
      <SettingItem
        onClick={() => navigate("/logs")}
        label={t("settings.sections.clash.shortcuts.logs")}
      />
      <SettingItem
        onClick={() => navigate("/unlock")}
        label={t("settings.sections.clash.shortcuts.tests")}
      />
      ...
      {!devUnlocked && (
        <SettingItem
          onClick={() => setDevPwdOpen(true)}
          label={t("settings.sections.clash.developer.entry")}
        />
      )}
        ...
        <>
          ...
          <SettingItem
            onClick={() => {
              ...
              showNotice.success(
                "settings.sections.clash.developer.exitSuccess",
              );
            }}
            label={t("settings.sections.clash.developer.exit")}
          />
        </>
      )}
    </SettingList>

      {devUnlocked && <SettingVergeAdvanced onError={onError} />}

      <Dialog open={devPwdOpen} onClose={() => setDevPwdOpen(false)} fullWidth maxWidth="xs">
        <DialogTitle>{t("settings.sections.clash.developer.dialogTitle")}</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
            type="password"
            label={t("settings.sections.clash.developer.passwordLabel")}
            fullWidth
            value={devPwdInput}
            onChange={(e) => setDevPwdInput(e.target.value)}
          />
        </DialogContent>
        <DialogActions>
          <Button
          ...
          >
            {t("settings.sections.clash.developer.cancel")}
          </Button>
          <Button
            variant="contained"
            onClick={() => {
              if (devPwdInput !== DEVELOPER_OPTION_PASSWORD) {
                showNotice.error("settings.sections.clash.developer.passwordError");
                return;
              }
          ...
          >
            {t("settings.sections.clash.developer.confirm")}
          </Button>
        </DialogActions>
      </Dialog>
    </>
