...
const PRELOAD_CONFIG_TIMEOUT_MS = 4000;
...
const withTimeout = <T,>(promise: Promise<T>, ms: number): Promise<T | null> => {
  return Promise.race([
    ...
    new Promise<null>((resolve) =>
      setTimeout(() => {
        console.warn("[preload.ts] Config fetch timed out, using fallback");
        resolve(null);
      }, ms),
    ),
  ]);
...
export const preloadConfig = async () => {
  try {
    const config = await withTimeout(getVergeConfig(), PRELOAD_CONFIG_TIMEOUT_MS);
    setPreloadConfig(config ?? null);
    return config ?? null;
  } catch (error) {
