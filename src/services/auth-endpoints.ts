...
function mergeConfig(...parts: (EndpointsConfig | null | undefined)[]): EndpointsConfig {
  ...
  const defaultBases = [defaults.primary, ...(defaults.backups ?? [])].filter(
...
/**
 * 公网 API 仅允许 HTTPS；本地开发允许 http://127.0.0.1:3001。
 * 禁止回退到 http://公网IP 或 http://域名:3001（会触发服务端 REQUIRE_HTTPS）。
 */
function isInsecureApiBase(url: string): boolean {
  try {
    ...
    if (!isLocalDevBase(url)) return true;
    return u.port !== "3001" && u.port !== "";
  } catch {
    return true;
  }
}
...
/** 清除本地线路缓存（登录页「清除」或设置中调用），下次启动重新探测 */
export function clearAuthEndpointsCache(): void {
  if (canUseStorage()) {
  ...
  initDone = false;
  initPromise = null;
  endpointsConfig = readBuiltinDefaults();
  activeBaseUrl = normalizeAuthBase(endpointsConfig.primary);
  orderedBases = [];
}
